...
public class CopyOfHtmlMerge {

	HashMap<Double, HtmlReportValues> report=new HashMap<Double, HtmlReportValues>();
...
//		htmlReport("WindowsIE");
		...
		htmlMergeReport();
		Set<Double> keySet = report.keySet();
...
//			String winIEStatus=report.get(key).winIEStatus;
			System.out.println("Test Key ID====>>"+key);
			System.out.println("====================================*****************************================================");
...
//			System.out.println("windows IE status=============>>>>>>"+winIEStatus);
			System.out.println("====================================*****************************================================");
			htmlMERREPROT(key,report.get(key).desc,winGCStatus,winMFStatus);
		}
		...
		for (int i = 1; i <= tabSize; i++) {
			...
			try{	
				...
				if(!report.containsKey(testLinkId)){
					...
					Double testLinkID = Double.parseDouble(htmlReportValues.testLinkID.replace("WHOIS-", ""));
					System.out.println("TestLInk ID While Adding====>>>"+testLinkID);
					report.put(testLinkID,htmlReportValues );
				}
				else{
					HtmlReportValues reportValues = report.get(testLinkId);
					reportValues = recordResult(reportValues, result, platform);
					Double testLinkID = Double.parseDouble(testLinkId.replace("WHOIS-", ""));
					System.out.println("TestLInk ID While Adding====>>>"+testLinkID);
...
//				report.put(htmlReportValues.testLinkID,htmlReportValues );	
	...
	private void htmlMERREPROT(Double tlid,String desc,String winGC,String winMF) throws Exception{
		FileWriter fstream = null;
		BufferedWriter out = null;
		newTest = true;
